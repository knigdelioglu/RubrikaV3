...
## Agent discipline addendum
- Read the current architecture and docs before editing code.
- Keep workflow, model status, job status, and project state connected.
- Buttons must call real backend commands or jobs.
- Silent no-op, fake success, and unreported test skips are forbidden.
- Raw technical errors must stay out of the main UI.
- QEP frozen rules and missing-rubric gates may not be weakened.
