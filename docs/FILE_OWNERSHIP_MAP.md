...
| **Performans görevi ve rubrik (TDE şablonları, sürümleme)** | `performanceOrganizationUi.ts`, `PerformanceOrganizationPage.tsx`, `performance_service.rs`, `performance_commands.rs`, `domain/performance.rs`, `domain/assessment.rs` | `PERFORMANCE_TEMPLATES` kataloğu (9. sınıf pilot), `RubricTemplateCatalog`/`RubricDraftEditor` entegrasyonu, rubrik sürüm yayınlama ve kilit, `create/update/list/get_performance_task`, `publish_performance_rubric`, `get_performance_rubric_history` | Yazılı sınav `ScoringRecord` akışı, QEP/OCR pipeline, mevcut rubrik puanlama policy’si | `npm run typecheck`, `cargo clippy`, `cargo test` |
| **Performans değerlendirme (ölçüt işaretleme, onay, Missing/NotPerformed)** | `PerformanceScoringPage.tsx`, `performanceOrganizationUi.ts`, `performance_service.rs`, `performance_commands.rs` | `save/approve/set_status/list_performance_assessments`, geçici toplam (servis tarafı), onay kuralları, eksik durumların sıfırdan ayrı yönetimi, sınıf listesi → öğrenci → düzey işaretleme UI’ı | Yazılı sınav puanlama/job akışı, `ScoringRecord`, otomatik puan üretimi (AI) | `npm run typecheck`, `cargo clippy`, `cargo test` |
| **Performans sonuç raporu (PDF/Excel)** | `PerformanceScoringPage.tsx` (`PerformanceResultsView`), `performanceReportUi.ts`, `performance_service.rs` (`get_performance_report`), `performance_commands.rs`, `index.css` | `get_performance_report` DTO (salt-okunur, öğrenci sabitlediği rubrik sürümüyle çözülür), PDF yazdırma görünümü (`window.print()`), UTF-8 BOM’lu noktalı virgül ayraçlı CSV indirme; Missing/NotPerformed ayrı gösterim | Yazılı sınav rapor/analysis akışı, `AnalysisPage`, `finish_assessment` job’u, `ScoringRecord` | `npm run build`, `npm run typecheck`, `cargo clippy` |
